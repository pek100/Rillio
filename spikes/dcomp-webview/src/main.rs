//! Stage 1 of the DComp compositing spike (see memory/compositing-dcomp-plan.md).
//!
//! Proves the unknown that gates the whole approach: can we GPU-composite a
//! *transparent* WebView2 (composition mode) on top of a D3D11 layer via
//! DirectComposition, in OUR toolchain (windows 0.61 + webview2-com 0.38), with
//! working hit-testing — the way the official Stremio Qt shell composites its
//! WebEngineView over the mpv FBO.
//!
//! Layer stack (bottom -> top):
//!   * a composition swapchain cleared to teal  = the future mpv video texture
//!   * a transparent WebView2 root visual        = the future web UI overlay
//!
//! Stage 2 swaps the teal swapchain for mpv's render-API output; Stage 3 ports
//! the proven path into the shell.
...
/// Everything the window proc needs to react to input / resize.
struct AppState {
    ...
    // Kept alive for the visual tree's lifetime.
...
impl AppState {
    /// Clear the composition swapchain to teal and present. This is the
    /// stand-in for the mpv-rendered video texture (Stage 2 replaces it).
    unsafe fn paint_teal(&self) -> Result<()> {
        let backbuf: ID3D11Texture2D = self.swapchain.GetBuffer(0)?;
        let mut rtv: Option<ID3D11RenderTargetView> = None;
        self.d3d
            .CreateRenderTargetView(&backbuf, None, Some(&mut rtv))?;
        let rtv = rtv.unwrap();
        let teal = [0.02f32, 0.30, 0.36, 1.0];
        self.ctx.ClearRenderTargetView(&rtv, &teal);
...
fn main() -> Result<()> {
    unsafe {
        ...
        let d3d = d3d.expect("d3d device");
        let ctx = ctx.expect("d3d context");
        println!("d3d11 device created");

        // --- composition swapchain (the video placeholder) ----------------
        ...
        println!("composition swapchain created");

        // --- DirectComposition device + visual tree ------------------------
        ...
        // Empty visual that WebView2 will render its transparent content into.
        let web_visual = dcomp.CreateVisual()?;
        root.AddVisual(&web_visual, true, &video_visual)?;
        println!("dcomp visual tree built");

        // --- WebView2 in composition mode ----------------------------------
        ...
        println!("webview2 composition controller attached to dcomp visual");
        ...
        // WebView2's host surface paints an opaque WHITE backing behind the page
        // by default — a transparent HTML body is not enough. Set the controller's
        // default background to fully transparent (alpha 0) so the DComp video
        // layer below shows through the page's transparent regions.
        ...
        drop(webview); // navigation already queued
        println!("entering message loop — click the button, resize the window");
...
/// Register the class and create a normal (opaque) top-level window. The window
/// itself need not be transparent — DComp paints the whole client area.
unsafe fn create_window() -> Result<HWND> {
    let hinstance = GetModuleHandleW(None)?;
    // Keep the UTF-16 buffers alive across the Register/Create calls — storing a
    // dangling .as_ptr() into WNDCLASSW is a use-after-free.
    let class_name = wide("DCompSpike");
    let title = wide("DComp + WebView2 spike (Stage 1)");
    let class = WNDCLASSW {
...
/// Synchronously create the WebView2 environment (pumps messages internally).
...
/// Synchronously create the composition controller for `hwnd`.
...
/// Navigate the webview to the local overlay.html sitting next to Cargo.toml.
unsafe fn navigate_overlay(controller: &ICoreWebView2Controller) -> Result<()> {
    ...
    let webview = controller.CoreWebView2()?;
    let wide = wide(&url);
    webview.Navigate(PCWSTR(wide.as_ptr()))?;
    Ok(())
}
...
extern "system" fn wndproc(hwnd: HWND, msg: u32, wparam: WPARAM, lparam: LPARAM) -> LRESULT {
    unsafe {
        ...
        match msg {
            WM_ERASEBKGND => LRESULT(1), // DComp owns the pixels; skip flicker.
            WM_SIZE => {
            ...
            WM_MOUSEWHEEL => {
                if let Some(state) = state {
                    // Wheel point is in screen coords; convert to client.
