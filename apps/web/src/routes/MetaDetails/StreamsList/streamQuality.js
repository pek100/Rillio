...
const parseSeeders = (text) => {
    // Torrentio: "👤 16". Others: "Seeders: 16" / "16 seeds".
    let m = /👤\s*(\d+)/.exec(text) || /(?:seeders?|seeds)\D{0,3}(\d+)/i.exec(text) || /(\d+)\s*(?:seeders?|seeds)/i.exec(text);
...
const parseHdr = (text) => /\b(hdr10\+?|hdr|dolby\s*vision|dovi|\bdv\b)\b/i.test(text);
...
const TIERS = [
    { key: '4k-hdr', label: '4K HDR', match: (q) => q.resolution >= 2160 && q.hdr },
    { key: '1080-hdr', label: '1080p HDR', match: (q) => q.resolution === 1080 && q.hdr },
    { key: '4k', label: '4K', match: (q) => q.resolution >= 2160 && !q.hdr },
    { key: '1080', label: '1080p', match: (q) => q.resolution === 1080 && !q.hdr },
];

const bySizeDesc = (a, b) => (b.quality.size || 0) - (a.quality.size || 0);
const bySeedersDesc = (a, b) => (b.quality.seeders || 0) - (a.quality.seeders || 0);
...
const curateStreams = (streams, lang) => {
    const withQuality = streams.map((stream) => ({ stream, quality: parseStream(stream) }));
    const byLangThenSize = (a, b) => (langScore(b.quality, lang) - langScore(a.quality, lang)) || bySizeDesc(a, b);
    const used = new Set();
    const picks = [];
...
const recommendStream = ({ picks, rest }, preset, screen, lang) => {
    ...
    if (preset === 'auto' && screen) {
        ...
        const ideal = pool.find((p) => {
            ...
            const res = q.resolution >= 2160 ? 2160 : (q.resolution === 1080 ? 1080 : q.resolution);
...
const availableLanguages = (streams) => {
    const out = new Set();
